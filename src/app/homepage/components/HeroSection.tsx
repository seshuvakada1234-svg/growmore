...
export default function HeroSection() {
  ...
  return (
    <section className="relative w-full overflow-x-hidden min-h-[480px] lg:min-h-[clamp(520px,80vh,720px)] flex items-center">
      {/* Background Image */}
      ...
      {/* Content Container - Forced Horizontal Layout */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 lg:px-12 w-full">
        <div className="flex flex-row items-center gap-4 lg:gap-12 w-full py-8 lg:py-16">
          
          {/* Left: Text Content */}
          <div className="flex-1 min-w-0 text-left">
            {/* Badge */}
            <div className={`inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm border border-white/30 rounded-full px-2 lg:px-4 py-1 mb-3 lg:mb-5 transition-opacity duration-500 ${transitioning ? 'opacity-0' : 'opacity-100'}`}>
              <span className="text-[10px] lg:text-sm font-semibold text-white truncate">{slide.badge}</span>
            </div>
            ...
            {/* CTAs - Scaled but Row Layout */}
            <div className={`flex flex-row items-center gap-2 lg:gap-3 transition-all duration-500 delay-200 ${transitioning ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
              <Link href={slide.ctaHref} className="inline-flex items-center justify-center gap-1 lg:gap-2 bg-white text-primary font-bold px-3 lg:px-6 py-2 lg:py-3.5 rounded-lg hover:bg-[#F1F8E9] transition-all text-[10px] lg:text-base whitespace-nowrap">
                {slide.cta}
                <ArrowRight className="h-3 w-3 lg:h-4 lg:w-4" />
              </Link>
              <Link href="/plants" className="inline-flex items-center justify-center gap-1 lg:gap-2 bg-white/15 backdrop-blur-sm border border-white/30 text-white font-semibold px-3 lg:px-6 py-2 lg:py-3.5 rounded-lg hover:bg-white/25 transition-all text-[10px] lg:text-base whitespace-nowrap">
                View All
              </Link>
            </div>

            {/* Slide Indicators */}
            <div className="flex justify-start gap-1.5 lg:gap-2 mt-4 lg:mt-8">
              {HERO_SLIDES.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setCurrentSlide(i)}
                  className={`h-1 lg:h-1.5 rounded-full transition-all duration-300 ${i === currentSlide ? 'w-4 lg:w-8 bg-[#A5D6A7]' : 'w-2 lg:w-4 bg-white/40'}`}
                />
              ))}
            </div>
          </div>

          {/* Right: Floating Cards - Always Visible and Horizontal */}
          <div className="flex flex-col gap-2 lg:gap-4 flex-shrink-0">
            ...
            {FLOATING_CARDS.map((card, i) => (
              <div
                key={i}
                className="bg-white/10 backdrop-blur-sm border border-white/15 rounded-lg lg:rounded-xl px-2 lg:px-4 py-1.5 lg:py-3 flex items-center gap-2 lg:gap-3 min-w-[120px] lg:min-w-[200px]"
                style={{
