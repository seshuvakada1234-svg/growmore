    ...
    >
      <div className="relative aspect-square overflow-hidden bg-muted">
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1 z-10">
          {plant.isBestseller && (
            <span className="text-[8px] md:text-[10px] bg-[#FF6F00] text-white font-black px-2 py-0.5 rounded shadow-sm uppercase tracking-wider">BESTSELLER</span>
          )}
          {plant.isNew && (
            <span className="text-[8px] md:text-[10px] bg-primary text-white font-black px-2 py-0.5 rounded shadow-sm uppercase tracking-wider">NEW</span>
          )}
          {discount > 0 && (
            <span className="text-[8px] md:text-[10px] bg-destructive text-white font-black px-2 py-0.5 rounded shadow-sm uppercase tracking-wider">{discount}% OFF</span>
          )}
        </div>

        <div className="absolute top-2 right-2 z-10 flex flex-col gap-1.5">
          <button 
            className="p-1.5 md:p-2 rounded-full transition-all bg-white/60 hover:bg-white backdrop-blur-sm shadow-sm"
          ...
          <ShareButton 
            product={plant} 
            className="p-1.5 md:p-2 h-auto w-auto rounded-full bg-white/60 hover:bg-white backdrop-blur-sm text-muted-foreground shadow-sm" 
            variant="ghost"
          />
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-2 md:p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300 z-10">
          <button 
            onClick={addToCart}
            className={`w-full py-2 md:py-2.5 rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold uppercase tracking-widest transition-all shadow-lg h-[36px] md:h-auto ${addedToCart ? 'bg-primary text-white' : 'bg-white/95 text-primary hover:bg-primary hover:text-white'}`}
          >
      ...
      <div className="p-3 md:p-4">
        <div className="flex items-start justify-between gap-2 mb-1">
          <h3 className="font-headline font-bold text-[#1A2E1A] text-xs md:text-sm leading-tight line-clamp-2 group-hover:text-primary transition-colors min-h-[32px]">
            {plant.name}
          </h3>
        </div>
        <p className="text-[10px] md:text-xs text-muted-foreground mb-2 md:mb-3">{plant.category}</p>

        <div className="flex items-center justify-between mb-2 md:mb-3">
          <div className="flex items-center gap-1">
            <div className="flex items-center gap-0.5 bg-primary text-white text-[9px] md:text-[10px] font-bold px-1.5 py-0.5 rounded">
              <span>{plant.rating}</span>
              <Star className="h-2 w-2 md:h-2.5 md:w-2.5 fill-current" />
            </div>
            <span className="text-[9px] md:text-[10px] text-muted-foreground font-medium">({plant.reviewCount})</span>
          </div>
          <div className="flex items-center gap-1.5 md:gap-2">
            <span className="font-bold text-primary text-sm md:text-base">{formatPrice(plant.price)}</span>
            {plant.oldPrice && (
              <span className="text-[10px] md:text-xs text-muted-foreground line-through">{formatPrice(plant.oldPrice)}</span>
            )}
          </div>
        </div>

        <div className="flex items-center justify-between mt-auto pt-2 md:pt-3 border-t border-dashed border-border">
          <div className={`px-1.5 md:px-2 py-0.5 rounded-full text-[9px] md:text-[10px] font-bold ${careLevelColor}`}>
            {careLevelLabel}
          </div>
          <p className="text-[9px] md:text-[10px] text-muted-foreground font-semibold flex items-center gap-1">
...
export default function ProductGrid({ title, subtitle, filterKey, limit = 8, showViewAll = true, viewAllHref = '/plants' }: ProductGridProps) {
  ...
  return (
    <section className="py-10 md:py-16 bg-neutral/30">
      <div className="max-w-7xl mx-auto px-4 lg:px-12">
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-6 md:mb-8 gap-4">
          <div>
            {subtitle && <div className="text-primary font-bold uppercase tracking-wider text-[10px] md:text-xs mb-1">{subtitle}</div>}
            <h2 className="text-2xl md:text-3xl font-headline font-extrabold text-[#1A2E1A]">{title}</h2>
          </div>
          {showViewAll && (
            <button 
              onClick={() => router.push(viewAllHref)}
              className="flex items-center gap-2 text-xs md:text-sm font-bold text-primary hover:underline group"
            >
