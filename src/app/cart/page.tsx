...
import { Trash2, Minus, Plus, ArrowRight, ShoppingBag, ShieldCheck, Loader2 } from "lucide-react";
...
export default function CartPage() {
  ...
  // ── Enrich cart items with product details from Firestore ─────────────────
    ...
    try {
      ...
      const enriched = await Promise.all(
        cartItems.map(async (item) => {
          ...
          const mock = PRODUCTS.find((p) => p.id === item.id);
          if (mock) {
            return { ...mock, imageUrl: resolveImage(mock.imageUrl, 300), quantity: item.quantity };
          }
          return null;
        })
      );
  ...
  useEffect(() => { enrichItems(); }, [enrichItems]);
